...
@Component({
    ...
    standalone: true,
    imports: [RouterModule, DialogModule, ConfirmDialogModule, InputTextModule, InputIconModule, IconFieldModule, ButtonModule, TableModule, CommonModule, BadgeModule, RatingModule, FormsModule, TagModule, ImageModule],
    template: `
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                <thead class="bg-gray-100 text-gray-700 text-left">
                    <tr>
                        ...
                        <th class="p-3 max-w-[170px]">Tiêu đề</th>
                ...
                <tbody>
                    <tr *ngFor="let course of courses" class="border-t hover:bg-gray-50">
                        ...
                        <td class="p-3 max-w-[170px] break-words">
                        ...
                        <td class="p-3 space-x-2">
                            ...
                            <button class="px-3 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700" (click)="removeCourse(course.id)">Gỡ bỏ</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
export class CourseAcceptComponent implements OnInit {
    ...
    removeCourse(courseId: number) {
        Swal.fire({
            title: 'Xác nhận?',
            text: 'Bạn sẽ không thể thay đổi sau khi gỡ bỏ khóa học!',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                this.http.post(`http://localhost:8080/course/remove/${courseId}`, {}).subscribe((res) => {
                    this.loadCourses();
                    this.toastService.addToast('success', 'Khóa học đã được gỡ bỏ');
                });
            }
        });
    }
