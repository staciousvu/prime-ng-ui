...
@Component({
    ...
    template: `
        ...
        <div class="font-semibold text-xl mb-4">Quản lí voucher</div>
        <div class="card">
            <p class="text-lg font-semibold text-gray-800 mb-4">Voucher đang hoạt động</p>
            ...
            <!-- test -->
            <p-table [value]="vouchers1" [rows]="10" [tableStyle]="{ 'min-width': '70rem' }" [rowsPerPageOptions]="[10, 15, 20]" [(selection)]="selectedVouchers" [scrollable]="true">
                <ng-template #header>
                    <tr>
                ...
                <ng-template #body let-voucher>
                    <tr>
                        ...
                        <td>
                            <p-tag [value]="voucher.isActive ? 'Đang hoạt động' : 'Không hoạt động'" [severity]="getSeverityStatus(voucher.isActive)"></p-tag>
                        </td>
                        <td>
                            <p-toggleSwitch [(ngModel)]="voucher.isActive" (ngModelChange)="toggleStatus(voucher)"> </p-toggleSwitch>
                        </td>
                    </tr>
                </ng-template>
            </p-table>
        ...
        <div class="card">
          <p class="text-lg font-semibold text-gray-800 mb-4">Voucher chưa kích hoạt</p>
            ...
            <!-- test -->
            <p-table [value]="vouchers2" [paginator]="true" [rows]="5" [tableStyle]="{ 'min-width': '70rem' }" [rowsPerPageOptions]="[5, 10, 15]" [(selection)]="selectedVouchers" [scrollable]="true">
                <ng-template #header>
                    <tr>
        ...
        <div class="card">
          <p class="text-lg font-semibold text-gray-800 mb-4">Voucher đã hết hạn</p>
            ...
            <!-- test -->
            <p-table [value]="vouchers3" [paginator]="true" [rows]="10" [tableStyle]="{ 'min-width': '70rem' }" [rowsPerPageOptions]="[10, 15, 20]" [(selection)]="selectedVouchers" [scrollable]="true">
                <ng-template #header>
                    <tr>
                        <th style="min-width: 10rem">Code</th>
                        <th style="min-width: 10rem">Mô tả</th>
...
export class VoucherComponent implements OnInit {
    ...
    toggleStatus(voucher: any) {
        ...
        this.voucherService.toggleActiveVoucher(voucher.id).subscribe((response) => {
            this.loadData();
        });
        // this.loadData();
    }
    ...
    loadData() {
        ...
        this.voucherService.getInactiveVoucherNonExpired().subscribe((response) => {
            this.vouchers2 = response.data;
        });
        this.voucherService.getInactiveVoucherExpired().subscribe((response) => {
            this.vouchers3 = response.data;
        });
        this.voucherService.getInactiveNotStarted().subscribe((response) => {
