                ...
                <div class="card bg-white p-6 rounded shadow-md flex flex-col gap-5">
                    ...
                    <div class="flex flex-wrap gap-6">
                        <div class="flex flex-col grow basis-0 gap-2">
                            ...
                            <p-select [options]="discountTypes" [(ngModel)]="selectedDiscountType" optionLabel="label" optionValue="value" placeholder="Select discount type" class="w-full" />
                        </div>
                        <div class="flex flex-col grow basis-0 gap-2">
                ...
                <div class="flex flex-row gap-6 flex-1">
                    ...
                    <div class="w-1/2 flex justify-center items-center">
                        <div class="voucher-preview bg-white p-4 rounded-lg shadow-md w-full text-center">
                            <h2 class="text-lg font-bold text-green-600">{{ selectedDiscountType === 'PERCENTAGE' ? discountValue + '%' : '$' + discountValue }} OFF</h2>
                            <p class="text-md font-semibold mt-2">{{ code || 'VOUCHER_CODE' }}</p>
                            <p class="text-sm text-gray-600 mt-2">{{ description || 'Enter description' }}</p>
                            <p class="text-xs text-gray-500 mt-2">Valid from {{ startDate | date: 'shortDate' }} to {{ endDate | date: 'shortDate' }}</p>
                        </div>
                    </div>
...
export class AddVoucherComponent{
    ...
    createVoucher() {
        ...
        console.log('voucherrrrrrrr:',voucher);

        this.voucher.createVoucher(voucher).subscribe({
            next: (res) => this.toastService.addToast('success', 'Thêm voucher thành công'),
